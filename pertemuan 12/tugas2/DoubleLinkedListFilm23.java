...
public class DoubleLinkedListFilm23 {
    Film23 head;
    int size;

    public DoubleLinkedListFilm23() {
        head = null;
        size = 0;
    }
    ...
    public void addFirst(int id, String judul, double rate) {
        if (isEmpty()) {
            head = new Film23(null, id, judul, rate, null);
        } else {
            Film23 newNode = new Film23(null, id, judul, rate, head);
            head.prev = newNode;
            head = newNode;
        }
        size++;
    ...
    public void addLast(int id, String judul, double rate) {
        if (isEmpty()) {
            ...
            Film23 newNode = new Film23(current, id, judul, rate, null);
    ...
    public void add(int id, String judul, double rate, int index) throws Exception {
        if (isEmpty()) {
            ...
            if (current.prev == null) {
                Film23 newNode = new Film23(null, id, judul, rate, current);
                current.prev = newNode;
                head = newNode;
            } else {
                Film23 newNode = new Film23(current.prev, id, judul, rate, current);
    ...
    public Film23 findFilm(int key) {
        Film23 temp = head;
        while (temp != null) {
            if (temp.id == key) {
                break;
